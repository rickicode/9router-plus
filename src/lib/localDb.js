...
const DEFAULT_CHAT_RUNTIME_SETTINGS = Object.freeze({
  ...
  compactUpstreamTimeoutMs: 300_000,
...
function normalizeMorphApiKeyEntry(value, index = 0) {
  ...
  return {
    email,
    key,
    status: ["active", "inactive", "exhausted", "unknown"].includes(value.status)
      ? value.status
      : "unknown",
...
const DEFAULT_SETTINGS = {
  cloudEnabled: false,
  // cloudUrls is intentionally empty by default. The global cloudSharedSecret
  // is owned by 9Router and reused for every registered worker until it is
...
export function normalizeChatRuntimeSettings(settings = {}) {
  ...
  return {
    ...
    compactUpstreamTimeoutMs: normalizePositiveInteger(source.compactUpstreamTimeoutMs, DEFAULT_CHAT_RUNTIME_SETTINGS.compactUpstreamTimeoutMs),
