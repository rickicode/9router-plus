...
async function resolveMorphRuntimeId(request, env) {
...
function getMorphApiKeys(morphSettings) {
	return Array.isArray(morphSettings?.apiKeys)
		? morphSettings.apiKeys.filter(
				(entry) => entry?.key && entry.isActive !== false,
			)
		: [];
}
...
function getMorphKeyOrder(runtimeId, morphSettings) {
...
function selectMorphApiKey(runtimeId, morphSettings) {
	const apiKeys = getMorphKeyOrder(runtimeId, morphSettings);
	...
	async fetch(request, env, ctx) {
		...
		try {
			...
			if (path === "/v1/embeddings" && request.method === "POST") {
				const response = await handleEmbeddings(request, env, ctx);
				log.response(response.status, Date.now() - startTime);
				return addCorsHeaders(response);
			}
			...
			// No worker-side R2 backup routes remain. 9router-plus handles backup
