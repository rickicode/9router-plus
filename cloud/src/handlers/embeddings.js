 ...
 */
export async function handleEmbeddings(request, env, ctx) {
	...
	while (retryCount < MAX_RETRIES) {
		...
		try {
			const apiKey = extractBearerToken(request);
			const connection = await selectCredential(
				data,
				provider,
				apiKey || "default",
				env,
...
async function validateApiKey(request, runtimeId, env) {
...
async function markAccountUnavailable(
	...
	log.warn(
		"EMBEDDINGS_ACCOUNT",
		`${connectionId} | unavailable until ${conn.nextRetryAt}`,
	);
}
