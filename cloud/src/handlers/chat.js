...
function getMorphCompactCooldownKey(runtimeId, entry) {
	const keyId = getMorphCompactKeyId(entry);
	return keyId ? `${runtimeId}:${keyId}` : null;
}

function isMorphCompactKeyCooledDown(runtimeId, entry) {
	const cooldownKey = getMorphCompactCooldownKey(runtimeId, entry);
...
function setMorphCompactKeyCooldown(runtimeId, entry, status) {
	const cooldownKey = getMorphCompactCooldownKey(runtimeId, entry);
	if (!cooldownKey) return;
	const durationMs =
		status === 401 ? 30 * 60 * 1000 : status === 429 ? 60 * 1000 : 0;
	if (durationMs > 0) {
		morphCompactKeyCooldowns.set(cooldownKey, Date.now() + durationMs);
	}
}

function clearMorphCompactKeyCooldown(runtimeId, entry) {
	const cooldownKey = getMorphCompactCooldownKey(runtimeId, entry);
	if (cooldownKey) morphCompactKeyCooldowns.delete(cooldownKey);
}

function getMorphCompactKeyOrder(runtimeId, morphSettings) {
	...
	const availableKeys = apiKeys.filter(
		(entry) => !isMorphCompactKeyCooledDown(runtimeId, entry),
	);
	const eligibleKeys = availableKeys.length > 0 ? availableKeys : apiKeys;
	if (eligibleKeys.length <= 1 || morphSettings?.roundRobinEnabled !== true)
		return eligibleKeys;

	const currentIndex = morphCompactRotationCursors.get(runtimeId) || 0;
	const selectedIndex = currentIndex % eligibleKeys.length;
	morphCompactRotationCursors.set(
		runtimeId,
		(selectedIndex + 1) % eligibleKeys.length,
	);
	return eligibleKeys.map(
		(_, offset) => eligibleKeys[(selectedIndex + offset) % eligibleKeys.length],
	);
}
...
async function maybeAutoCompactBody(body, data, runtimeId) {
	const startedAt = Date.now();
	const plan = buildAutoCompactPlan(body, data?.settings?.autoCompact);
	if (!plan.ok) {
		if (
			plan.reason !== "disabled" &&
			plan.reason !== "below minimum messages"
		) {
	...
	const apiKeys = getMorphCompactKeyOrder(runtimeId, morph);
	...
	log.info(
		"COMPACT",
		`Auto compact starting for ${plan.messages.length} messages`,
		{
	...
	for (const [index, entry] of apiKeys.entries()) {
		try {
			...
			if (!response.ok) {
				...
				setMorphCompactKeyCooldown(runtimeId, entry, response.status);
				if (
					index < apiKeys.length - 1 &&
					isMorphCompactRetryableStatus(response.status)
				) {
					continue;
				}
				recordCompactUsage({
				...
				log.warn(
					"COMPACT",
					`Auto compact skipped: Morph returned ${response.status}`,
				);
				return body;
			}
			...
			const compactedBody = applyCompactedMessages(
				body,
				plan.key,
				plan.entries,
				result.messages,
			...
			if (!compactedBody) {
				...
				log.warn(
					"COMPACT",
					"Auto compact skipped: Morph returned incompatible message shape",
				);
				return body;
			}

			clearMorphCompactKeyCooldown(runtimeId, entry);
			recordCompactUsage({
				status: response.status,
				startedAt,
				responsePayload: result,
			...
			log.info(
				"COMPACT",
				`Auto compact completed for ${plan.messages.length} messages`,
				{
					messages: plan.messages.length,
					tools: Array.isArray(compactedBody?.tools)
	...
	log.warn(
		"COMPACT",
		`Auto compact skipped: ${lastStatus ? `Morph returned ${lastStatus}` : lastError?.message || "no Morph key succeeded"}`,
	);
	return body;
}
...
export async function handleChat(request, env, ctx) {
	...
	body = await maybeAutoCompactBody(body, data, runtimeId);
